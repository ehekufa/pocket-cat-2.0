-- main.lua
-- Pocket Code for Love2D

local db = {}
local state = {current_screen="home", project_index=-1, actor_index=-1, modal_mode="", modal_input="", show_modal=false, is_running=false, picker_category="", game_actors={}, scroll_offset=0}

local colors = {
    bg={0.012,0.165,0.196}, header={0,0.239,0.298}, accent={0.243,0.710,0.820},
    fab={1,0.561,0}, brick_event={0.953,0.612,0.071}, brick_control={0.902,0.494,0.133},
    brick_motion={0.161,0.502,0.725}, brick_looks={0.486,0.702,0.259}, text={1,1,1}, card_bg={0,0.302,0.353}
}

local brick_colors = {event=colors.brick_event, control=colors.brick_control, motion=colors.brick_motion, looks=colors.brick_looks}

local library = {
    event={{id="ev_start", text="Start", cat="event", isHeader=true}},
    control={{id="c_wait", text="Wait", val=1, cat="control"},{id="c_forever", text="Forever", cat="control"},{id="c_repeat", text="Repeat", val=5, cat="control"}},
    motion={{id="m_x", text="Change X", val=30, cat="motion"},{id="m_y", text="Change Y", val=30, cat="motion"},{id="m_setx", text="Set X", val=0, cat="motion"},{id="m_sety", text="Set Y", val=0, cat="motion"},{id="m_rot", text="Rotate", val=15, cat="motion"}},
    looks={{id="l_size", text="Size", val=10, cat="looks"},{id="l_setsize", text="Set Size", val=100, cat="looks"}}
}

local T = {
    projects="Projects on device", help="Help", community="Catrobat Community",
    my_projects="My projects", objects="Objects", scripts="Scripts", categories="Categories",
    blocks="Blocks", event="Event", control="Control", motion="Motion", looks="Looks",
    start="Start", wait="Wait seconds", forever="Forever", rep="Repeat",
    change_x="Change X by", change_y="Change Y by", set_x="Set X", set_y="Set Y",
    rotate="Rotate by", change_size="Change size by", set_size="Set size %",
    end_loop="End of loop", exit="EXIT", cancel="CANCEL", ok="OK",
    new_project="New project name", new_object="New object", project="Project", actor="Actor", block="Block"
}

function getText(k) return T[k] or k end
...
function getDefaultData()
    return {{name="My Project", actors={{name="Cat", color={1,0.5,0}, scripts={}},{name="Ball", color={0.3,0.6,1}, scripts={}}}}}
end
...
function serialize(t)
    if type(t)=="table" then
        ...
        for k,v in pairs(t) do
            if type(k)=="string" then s=s.."[\""..escapeString(k).."\"]=" end
            s=s..serialize(v)..","
        end
        return s.."}"
...
function love.load()
    ...
    if love.filesystem.getInfo(font_path) then
        fonts={normal=love.graphics.newFont(font_path,14),big=love.graphics.newFont(font_path,20),title=love.graphics.newFont(font_path,18),header=love.graphics.newFont(font_path,16),small=love.graphics.newFont(font_path,12),large=love.graphics.newFont(font_path,40)}
    else
        fonts={normal=love.graphics.newFont(14),big=love.graphics.newFont(20),title=love.graphics.newFont(18),header=love.graphics.newFont(16),small=love.graphics.newFont(12),large=love.graphics.newFont(40)}
    end
...
function love.update(dt) if state.is_running then updateGame(dt) end end
...
function love.mousepressed(x,y,b) if b==1 then handleClick(x,y) end end
...
function drawHome()
    ...
    for i,k in ipairs(items) do
        ...
        love.graphics.setColor(colors.text)
        love.graphics.setFont(fonts.normal)
        love.graphics.print(getText(k),20,y+22)
...
function handleHomeClick(x,y)
    if x>=0 and x<=love.graphics.getWidth() and y>=260 and y<=329 then state.current_screen="projects" end
    checkFABClick(x,y)
end
...
function drawProjects()
    ...
    local cpr=2 cw=(w-45)/cpr ch=cw
    for i,p in ipairs(db) do
        local col=(i-1)%cpr row=math.floor((i-1)/cpr) x=15+col*(cw+15) y=75+row*(ch+15)
        ...
        love.graphics.setColor(colors.text)
        love.graphics.setFont(fonts.normal)
        local dn=p.name or getText("project")
        if #dn>15 then dn=dn:sub(1,12).."..." end
        love.graphics.print(dn,x+10,y+ch-40)
        love.graphics.setColor({1,0,0,0.5})
        love.graphics.circle("fill",x+cw-20,y+20,12)
        love.graphics.setColor(colors.text)
        love.graphics.setFont(fonts.small)
        love.graphics.print("x",x+cw-25,y+13)
        love.graphics.setFont(fonts.normal)
    end
    drawFAB()
end
...
function handleProjectsClick(x,y)
    local w=love.graphics.getWidth() cpr=2 cw=(w-45)/cpr ch=cw
    for i,p in ipairs(db) do
        local col=(i-1)%cpr row=math.floor((i-1)/cpr) cx=15+col*(cw+15) cy=75+row*(ch+15)
        if x>=cx and x<=cx+cw and y>=cy and y<=cy+ch then
            local dx=cx+cw-20 dy=cy+20
            if math.sqrt((x-dx)^2+(y-dy)^2)<=12 then
                table.remove(db,i) saveData() return
            else
                state.project_index=i state.current_screen="actors" return
            end
        end
    end
    checkFABClick(x,y)
...
function drawActors()
    local p=db[state.project_index]
    drawHeader(p.name or getText("objects"),true)
    for i,a in ipairs(p.actors) do
        local y=75+(i-1)*70
        love.graphics.setColor({0.1,0.2,0.25,0.3})
        love.graphics.rectangle("fill",0,y,love.graphics.getWidth(),69)
        love.graphics.setColor(a.color or {1,0,0})
        love.graphics.rectangle("fill",20,y+22,25,25)
        love.graphics.setColor(colors.text)
        love.graphics.setFont(fonts.normal)
        love.graphics.print(a.name or getText("actor"),60,y+28)
        love.graphics.setColor({1,0,0,0.5})
...
function handleActorsClick(x,y)
    ...
    for i,a in ipairs(p.actors) do
        local yp=75+(i-1)*70
        if x>=0 and x<=love.graphics.getWidth() and y>=yp and y<=yp+69 then
            local dx=love.graphics.getWidth()-30
            if math.sqrt((x-dx)^2+(y-(yp+35))^2)<=12 then
                table.remove(p.actors,i) saveData() return
            else
                state.actor_index=i state.current_screen="editor" return
            end
        end
    end
    checkFABClick(x,y)
...
function drawEditor()
    local a=db[state.project_index].actors[state.actor_index]
    drawHeader(a.name or getText("scripts"),true)
    local y=75-state.scroll_offset
    for i,s in ipairs(a.scripts or {}) do
        y=drawBrick(s,0,y)+5
    ...
    love.graphics.setColor(colors.accent)
    love.graphics.polygon("fill",love.graphics.getWidth()-60,h-55,love.graphics.getWidth()-60,h-25,love.graphics.getWidth()-30,h-40)
    love.graphics.setFont(fonts.normal)
end

function drawBrick(s,x,y)
    local c=brick_colors[s.cat] or {0.5,0.5,0.5}
    ...
    love.graphics.setColor(colors.text)
    love.graphics.setFont(fonts.normal)
    local t=s.text or getText("block")
    if t=="Start" then t=getText("start")
    elseif t=="Wait" then t=getText("wait")
    elseif t=="Forever" then t=getText("forever")
    elseif t=="Repeat" then t=getText("rep")
    elseif t=="Change X" then t=getText("change_x")
    elseif t=="Change Y" then t=getText("change_y")
    elseif t=="Set X" then t=getText("set_x")
    elseif t=="Set Y" then t=getText("set_y")
    elseif t=="Rotate" then t=getText("rotate")
    elseif t=="Size" then t=getText("change_size")
    elseif t=="Set Size" then t=getText("set_size") end
    love.graphics.print(t,x+75,y+18)
    if s.val~=nil then
        love.graphics.setColor(colors.text)
        love.graphics.rectangle("line",x+190,y+10,45,30)
        love.graphics.print(tostring(s.val),x+200,y+18)
    end
    love.graphics.setColor({1,0,0,0.5})
    love.graphics.print("x",x+250,y+18)
    if s.id=="ev_start" or s.id=="c_forever" or s.id=="c_repeat" then
        local sy=y+56
        love.graphics.setColor({1,1,1,0.1})
        love.graphics.rectangle("fill",x+40,sy,20,40)
        love.graphics.setColor({1,1,1,0.2})
        love.graphics.line(x+40,sy,x+60,sy+40)
        love.graphics.line(x+60,sy,x+40,sy+40)
        if s.children then
            local cy=sy
            for _,ch in ipairs(s.children) do cy=drawBrick(ch,x+40,cy)+5 end
            if s.id~="ev_start" then
                love.graphics.setColor(c)
                love.graphics.rectangle("fill",x+40,cy+5,226,48)
                love.graphics.setColor(colors.text)
                love.graphics.setFont(fonts.small)
                love.graphics.print(getText("end_loop"),x+50,cy+20)
                love.graphics.setFont(fonts.normal)
                return cy+53
            end
        else
            if s.id~="ev_start" then
                love.graphics.setColor(c)
                love.graphics.rectangle("fill",x+40,sy+5,226,48)
                love.graphics.setColor(colors.text)
                love.graphics.setFont(fonts.small)
                love.graphics.print(getText("end_loop"),x+50,sy+20)
                love.graphics.setFont(fonts.normal)
                return sy+53
            end
        end
...
function handleEditorClick(x,y)
    local h=love.graphics.getHeight() w=love.graphics.getWidth()
    if x>=10 and x<=50 and y>=h-60 and y<=h-20 then state.current_screen="categories" return end
    if x>=w-70 and x<=w-20 and y>=h-60 and y<=h-20 then playGame() return end
    if x>=10 and x<=50 and y>=10 and y<=50 then state.current_screen="actors" end
end

function drawCategories()
    drawHeader(getText("categories"),true)
    local cats={{name="event",color=colors.brick_event},{name="control",color=colors.brick_control},{name="motion",color=colors.brick_motion},{name="looks",color=colors.brick_looks}}
...
function handleCategoriesClick(x,y)
    local cats={"event","control","motion","looks"}
    for i,cat in ipairs(cats) do
        local yp=75+(i-1)*80
        if x>=0 and x<=love.graphics.getWidth() and y>=yp and y<=yp+79 then
            state.picker_category=cat state.current_screen="picker" return
        end
    end
...
function handlePickerClick(x,y)
    ...
    local yp=75
    for i,b in ipairs(blocks) do
        if x>=20 and x<=270 and y>=yp and y<=yp+56 then
            local a=db[state.project_index].actors[state.actor_index]
            local nb={id=b.id,text=b.text,cat=b.cat}
            if b.val then nb.val=b.val end
            if b.isHeader then nb.isHeader=true end
            if b.id=="ev_start" or b.id=="c_forever" or b.id=="c_repeat" then nb.children={} end
            table.insert(a.scripts,nb)
            saveData()
...
function drawStage()
    ...
    for _,a in ipairs(state.game_actors) do
        love.graphics.setColor(a.color)
        love.graphics.rectangle("fill",a.x-35,a.y-35,70,70)
        love.graphics.setColor(colors.text)
        love.graphics.setFont(fonts.normal)
        love.graphics.print(a.name,a.x-20,a.y-5)
    end
...
function handleStageClick(x,y)
    if x>=love.graphics.getWidth()-100 and x<=love.graphics.getWidth()-10 and y>=10 and y<=50 then stopGame() end
end
    ...
    if x>=w/2+140 and x<=w/2+200 and y>=310 and y<=350 then
        if #state.modal_input>0 then
            ...
            elseif state.modal_mode=="actor" then
                local cl={{1,0.5,0},{0.3,0.6,1},{0.8,0.2,0.8},{0,0.8,0.2},{1,0,0.5},{0.5,0.8,0}}
                table.insert(db[state.project_index].actors,{name=state.modal_input,color=cl[math.random(#cl)],scripts={}})
...
function playGame()
    ...
    local p=db[state.project_index]
    local w,h=love.graphics.getWidth(),love.graphics.getHeight()
    for _,a in ipairs(p.actors) do
        table.insert(state.game_actors,{name=a.name,color=a.color,scripts=a.scripts,x=w/2+math.random(-100,100),y=h/2+math.random(-100,100),s=1,r=0})
    end
    for _,a in ipairs(state.game_actors) do
        runCode(a,a.scripts)
    end
